...
export function StoreList() {
  ...
  const grocery = (stores ?? []).filter((s) => (s.kind ?? 'grocery') === 'grocery')
  const other = (stores ?? []).filter((s) => s.kind === 'other')

  const renderCard = (store: StoreType) => (
    <div
      key={store.id}
    ...
    >
      ...
      <span className="flex-1 font-display font-semibold text-espresso">{store.name}</span>
  ...
  const renderSection = (title: string, list: StoreType[]) =>
    list.length > 0 && (
      <div className="space-y-2">
        <p className="px-1 text-xs font-bold uppercase tracking-wide text-espresso/55">
          {title}
        </p>
        <div className="stagger grid gap-3 sm:grid-cols-2">{list.map(renderCard)}</div>
      </div>
    )
  ...
  return (
    <div className="space-y-5">
      ...
      {(stores?.length ?? 0) === 0 ? (
        ...
        <>
          {renderSection('Grocery Stores', grocery)}
          {renderSection('Other Stores', other)}
        </>
      )}

      {isFormOpen && <StoreForm store={editingStore} onClose={handleCloseForm} />}
    </div>
  )
}
